...
public class ServiceObjectProxy implements IAppService {
    ...
    public Angajat login(Angajat ang,Observer observer) throws ServiceException {
        ...
        sendRequest(new LoginAngajatRequest(ang.getUsername(),ang.getPassword()));
    ...
    public void deleteSpectacol(long id) throws ServiceException {
        testConnectionOpen();
        sendRequest(new DeleteSpectacolRequest(id));
        var response =readResponse();
        if(!(response instanceof DeleteSpectacolResponse)){
            throw new ServiceException("Expected DeleteSpectacolResponse, received:\n " + response);
        }
    }
