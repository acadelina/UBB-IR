...
contRDOnlyGUI::contRDOnlyGUI(contCRUDGUI* c) : cos{ c }
{
	this->build_UI();
	cos->service.addObserver(this);
}
void contRDOnlyGUI::build_UI()
{

	QHBoxLayout* mainly = new QHBoxLayout;
	this->setLayout(mainly);
}
void contRDOnlyGUI::update()
{
	this->repaint();
}

void contRDOnlyGUI::paintEvent(QPaintEvent* ev)
{
	...
	int x = -20;
	for (int i = 1; i <= cos->service.getContract().size(); i++)
	{
		x += 40;
